...
def create_short_simulations(models,factors, dc_range,data_folder='data',input_dir='inputs',inh_factor:np.ndarray=np.array([1])):
    ...
    for dc in dc_range:
        for mc,f,inh_f in zip(models,factors,inh_factor):
            ...
            commend = f"python3 -m simulations.simulate_neuron --neuron_model_folder simulations/neuron_models/{mc} --simulation_folder /ems/elsc-labs/segev-i/nitzan.luxembourg/projects/neuron_mi/neuron_mi/simulations/{data_folder}/{mc}_factor_{f_str}_inh_factor_{inh_f_str}_DC_{dc}  --DC_shift {dc}  --weight_scale_factor {f} --inh_weight_scale_factor {inh_f} --save_plots True --simulation_duration_in_seconds 60 --input_dir '/ems/elsc-labs/segev-i/nitzan.luxembourg/projects/neuron_mi/neuron_mi/simulations/{data_folder}/{input_dir}/'"
            print(commend)
...
create_short_simulations(['Rat_L5b_PC_2_Hay_noNMDA']*4,[1.]*4,list(range(-20,30,20)),data_folder='slim_data',inh_factor=np.array([1.]*4))