...
def drawTree2(blnCompact):
    ...
    def go(blnPruned, tree):
        ...
        def lmrBuild(w, f):
            def go(wsTree):
                ...
                def linked(s):
                    c = s[0]
                    t = s[1:]
                    return _x + T_INTERSECTION + t if R_INTERSECTION == c else (
                        _x + PIPE_INTERSECTION + t if BAR_INTERSECTION == c else (
                            _x + CROSS + t if F_PIPE_INTERSECTION == c else (
                                    _x + FLIP_T + t
                            )
                        )
                    )
                ...
                # CHILDREN --------------------------------
                    ...
                    return fghOverLMR(
                        ...
                        lmrFromStrings(
                            intercalate([] if blnCompact else [BAR_INT_2])(
                                [treeFix(' ', R_INTERSECTION, BAR_INT_2)(lmrs[0])] + [
                                    treeFix(BAR_INT_2, F_PIPE_INTER_2, BAR_INT_2)(x) for x
                                    in lmrs[1:-1]
                                ] + [treeFix(BAR_INT_2, F_R_INTER, ' ')(lmrs[-1])]
                            )
                        )
                    )
        ...
        return [
            ...
            if any(c not in BAR_INTER_3 for c in s)
